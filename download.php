...
function getAllLinks($Response, $domain)
{
    ...
    foreach ($aTagElements as $AnElement) {
        ...
        if (filter_var($aLink, FILTER_VALIDATE_URL) && strstr($aLink, $domain)) {
            ...
            if (strpos($aLink, 'download') > 0) {
                global $downloadsPath;
                global $Client;

                $isMp3 = saveMp3($Client->get($aLink), $downloadsPath, $aLink);
                if ($isMp3) {
                    continue;
                }
            }
...
function addNewLinks(array $newLinks, $links, $Response)
{
...
do {
    ...
    try {
        if (isset($links[$currentLinkIndex])) {
            ...
            if (!$isMp3) {
                $links = addNewLinks(getAllLinks($Response, $domain), $links, $Response);
            }
        } else {
